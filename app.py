...
with st.expander("🚀 批量合成教学样本", expanded=True):
    ...
    with col_right:
        if st.button("开始合成", type="secondary"):
            ...
            progress_bar = st.progress(0.0, text="正在构建大纲…")
            status = st.empty()

            def on_progress(event: dict) -> None:
                stage = event.get("stage")
                if stage == "outline":
                    progress_bar.progress(0.05, text=f"大纲就绪，共 {event.get('count', 0)} 节")
                elif stage == "prep":
                    progress_bar.progress(0.1, text=f"展开要点 {event.get('slots', 0)} 条，开始轮询生成…")
                elif stage == "sample":
                    done = event.get("completed", 0)
                    total = max(event.get("total", int(bulk_num)), 1)
                    ratio = min(done / total, 1.0)
                    eta = event.get("eta_seconds")
                    hint = f"，预计剩余 {eta:.1f}s" if eta is not None else ""
                    progress_bar.progress(ratio, text=f"正在生成 {done}/{total}{hint}")
                    status.write(
                        f"#{done} {event.get('section')} · 变体{event.get('variant')} · {event.get('difficulty')}"
                    )

            with st.spinner("正在生成教学样本..."):
                dataset = synth.build(
                    topic=bulk_topic,
                    num_questions=int(bulk_num),
